...
本文档面向看板的使用者，介绍看板界面、状态与动画的含义、nudge 催办机制，以及常见问题与调试方法。以「看板长什么样、每个元素在说什么」为线索，帮助你看懂 Claude Code 各 Agent 正在做什么。
...
当子 Agent 长时间没有动静时，看板会派一只红色小人来催办。

### 触发条件（需同时满足）

1. 该 Agent 是**子 Agent**（主 Agent 永不催办）；
2. 当前状态**不是** `done` / `failed` / `asking`（完成、失败、正在等你输入都不催）；
3. 距最近一次通讯时间（`lastSeen`）**超过 5 分钟**（`NUDGE_THRESHOLD_MS`）。

> 「静默」以事件为准：只要子 Agent 还在发事件（思考、调工具等），`lastSeen` 就会被刷新，不会触发催办。

### 表现形式

- 一只**红色 🤨 小人**从卡片左侧边缘外跑向卡片左缘（约 2.6 秒跑完全程）；
- 到达时刻在卡片右上角弹出红色气泡「**🤨 挑眉看你，快点！**」，约 2.6 秒后自动消失。

### 冷却

同一子 Agent 催办后进入 **5 分钟冷却**（`NUDGE_COOLDOWN_MS`）。即使它仍然静默，冷却期内不会重复派小人，避免 600ms 轮询导致高频触发。子 Agent 消失后催办记录自动清理。

### 不催办的情况

- 主 Agent；
- `done` / `failed` / `asking` 状态的子 Agent；
- 静默时长不足 5 分钟；
- 冷却期内；
- 页面首次渲染帧（首帧守卫）；
- 用户开启了系统「减弱动态效果」（`prefers-reduced-motion: reduce`）——此时**只弹气泡、不派小人**。
...
**Q2：催办怎么没触发？**
先检查是否符合触发条件：子 Agent 是否静默超过 5 分钟、是否处于 `done`/`failed`/`asking`、是否在 5 分钟冷却内。只要子 Agent 还在发事件（例如反复思考/调用工具），`lastSeen` 持续刷新，就不会催办——**有反馈就不催**。另外，开启系统「减弱动态效果」时只弹气泡不派小人，且页面首次渲染的那一帧不会催办。

**Q3：为什么主 Agent 恒显示「主 Agent」？**
这是前端的固定规则：主 Agent 卡片名称写死为「主 Agent」，不采用服务端可能误写的任务描述；子 Agent 名称才按「精确配对 → 最近派发描述 → 事件自带 prompt」的优先级解析。

**Q4：开启「减弱动态效果」会怎样？**
系统开启 `prefers-reduced-motion` 后：所有 CSS 动画（呼吸脉动、spinner、粒子、挥手等）被大幅缩短/禁用；火柴人动画不创建；催办只弹气泡。功能（状态展示、卡片更新）不受影响。
